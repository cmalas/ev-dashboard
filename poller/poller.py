...
Credit cost per poll cycle (The Odds API v4):
  ...
  Props:     fetched per-event via /events/{id}/odds
             ~7 markets x 1 region = 7 credits per game
             Only games within 24h are fetched for props
             Props polled every PROPS_CYCLE_INTERVAL mainline cycles (default 6)
             e.g. 8 games x 7 markets = 56 credits per props cycle
             ~8 props cycles/day = ~448 props credits/day = ~13K/month
             Total with mainlines (~9/cycle x 48/day = 13K) = ~26K... watch this.
             If credits run tight, raise PROPS_CYCLE_INTERVAL or lower PROPS_HOURS_AHEAD.
"""
...
# ALL books for mainline requests (10 = 1 region-equivalent = 3 credits/sport)
...
# Sports to fetch props for and their market keys
# Props use the /events/{id}/odds endpoint (per-event, costs more)
PROPS_SPORTS = {
    "baseball_mlb": [
        "batter_hits",
        "batter_home_runs",
...
# Only fetch props for games starting within this many hours
PROPS_HOURS_AHEAD = int(os.getenv("PROPS_HOURS_AHEAD", 24))

# Fetch props every N mainline cycles to manage credit usage.
# Default 6 = every 3 hours (at 30min mainline interval).
# Raise this if credits run low.
PROPS_CYCLE_INTERVAL = int(os.getenv("PROPS_CYCLE_INTERVAL", 6))

# Books to request props from — use regions param for props (cheaper)
...
def check_events_free(sport_key: str) -> int:
    """FREE /events endpoint — count upcoming games, zero credits."""
...
def fetch_events_free(sport_key: str) -> list[dict]:
    """
    FREE /events endpoint — returns event list with IDs and commence times.
    Used to find which games are coming up before spending credits on props.
    """
    url = f"{ODDS_API_BASE}/sports/{sport_key}/events"
    params = {"apiKey": ODDS_API_KEY}
    try:
...
def fetch_odds(sport_key: str) -> list[dict]:
    """Fetch mainline odds for a sport. Costs 3 credits."""
...
def fetch_event_props(sport_key: str, event_id: str, markets: list[str]) -> dict | None:
    ...
    Uses /events/{eventId}/odds endpoint — costs 1 credit per market per region.
    Returns the event dict with bookmakers, or None on failure.
    """
...
def compute_ev_for_event(cur, game_id: int, event: dict):
    """Mainline EV computation — h2h, spreads, totals."""
...
def extract_prop_lines(bookmakers: list[dict], market_key: str) -> dict:
    """
    Extract prop lines from all books for a given market.

    Props have a different structure from mainlines:
      outcome["name"]        = "Over" or "Under"
      outcome["description"] = player name, e.g. "Freddie Freeman"
      outcome["point"]       = the line, e.g. 1.5

    Returns:
      { player_name: { book_key: { "over": {price, point}, "under": {price, point} } } }
    """
    result = {}
    for bk in bookmakers:
        bk_key = bk["key"]
...
def compute_ev_for_props(cur, game_id: int, event_data: dict, sport_key: str):
    """
    Compute +EV for player prop markets.

    Strategy:
    - Use the consensus median of all available books as the "sharp" line
      (Pinnacle rarely has props; soft books set their own lines)
    - Devigify each book's Over/Under to get true probabilities
    - Compare each book's price against the consensus true prob
    - Store results with market_type = prop market key (e.g. "batter_hits")

    Note: Because no single sharp book dominates props the way Pinnacle does
    mainlines, we use a consensus approach across all available books.
    """
    bookmakers = event_data.get("bookmakers", [])
    if not bookmakers:
        return

    # Determine which prop markets are present in this event's response
    ...
    for market_key in prop_markets_present:
        ...
        for player, book_data in prop_lines.items():
            # Collect all books that have both Over and Under for this player
            books_with_both = {
            ...
            if len(books_with_both) < 2:
                continue  # Need at least 2 books to build consensus

            # Check all books use the same line (point value)
            # If books disagree on the line, skip — can't compare fairly
            points = set(
                sides["over"]["point"]
                for sides in books_with_both.values()
            )
            if len(points) > 1:
                # Multiple lines in market — only use books on the most common line
                from collections import Counter
                most_common_point = Counter(
            ...
            the_point = next(iter(books_with_both.values()))["over"]["point"]

            # Devigify each book's lines and collect true probabilities
            true_over_probs = []
            ...
            # Consensus true probability = median across all books
            ...
            # Now check each book's price against consensus
            for bk, sides in books_with_both.items():
                for side, true_prob, no_vig in [
                    ...
                    # outcome_name = "Over Freddie Freeman" for display
...
def poll_once(rds, cycle_number: int):
    ...
    cycle_number is used to decide whether to also fetch props this cycle.
    """
    ...
    try:
        ...
        if fetch_props_this_cycle:
            ...
            for sport_key, prop_markets in PROPS_SPORTS.items():
                ...
                # Use free events endpoint to find upcoming game IDs + times
                ...
                for ev in upcoming:
                    event_id = ev["id"]
                    try:
                        event_data = fetch_event_props(sport_key, event_id, prop_markets)
                    except Exception as e:
                        log.error(f"    Props fetch failed for {event_id[:8]}: {e}")
                        continue

                    if not event_data:
                        continue

                    # Ensure game row exists (may already be there from mainlines)
    ...
    credits_props     = props_fetched * len(next(iter(PROPS_SPORTS.values()), []))
    log.info(
        f"-- Poll complete: {sports_polled} sports polled, {sports_skipped} skipped, "
        f"{total_events} mainline events, {props_fetched} prop events. "
        f"Est. credits: mainlines ~{credits_mainlines}, props ~{credits_props} --\n"
    )
    ...
    summary = {
        ...
        "est_credits_cycle":   credits_mainlines + credits_props,
    }
...
def main():
    ...
    log.info(
        f"Poll interval: {POLL_INTERVAL}s | Sports: {', '.join(SPORTS)} | "
    ...
    while True:
        ...
        try:
            ...
            cycle_number += 1  # still increment so props timing isn't thrown off
